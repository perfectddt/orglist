...
### OP-LAN-FOLDER：手机局域网读取电脑文件夹

- 双击 `启动Org清单-手机WebDAV.cmd`；电脑可以接网线，手机可以接 Wi-Fi，只要位于同一路由器局域网。
- 在自动打开的电脑页面点击“打开 Org 文件夹”并授予读写权限，页面会把当前 Org 文件发布到带随机令牌的桥接内存。
- 手机打开命令窗口显示的完整地址后会自动载入电脑共享文件，不经过坚果云或其他 WebDAV 服务。
- 手机写回由电脑页面接收，再通过浏览器文件句柄写回原文件；电脑页面与 CMD 必须保持开启。
- 电脑页面停止响应约 12 秒后共享失效；桥接进程关闭后内存副本消失。
