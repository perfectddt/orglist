/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"4.1.13","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "4.1.13", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "布局按钮按下立即切换，不再依赖容易丢失的 pointerup" },
    activate(api) {
      ...
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "4.1.13", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function bindNode(element, node) {
        ...
        layoutButton?.addEventListener("pointerdown", (event) => { if (event.isPrimary === false || (event.pointerType === "mouse" && event.button !== 0)) return; event.preventDefault(); event.stopPropagation(); suppressNextLayoutClick = true; window.clearTimeout(layoutClickGuardTimer); layoutClickGuardTimer = window.setTimeout(() => { suppressNextLayoutClick = false; }, 500); activateLayoutButton(); });
