/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"4.1.8","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-core-layout{min-width:64px;min-height:48px}.owb-layout-open{min-width:52px;min-height:40px;font-size:11px;padding:9px 10px}
  `;
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "4.1.8", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "修复导航容器中的布局按钮把轻微触摸抖动误判成拖动" },
    activate(api) {
      ...
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "4.1.8", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function bindNode(element, node) {
        ...
        element.onpointerdown = (event) => { if (event.target.closest(".owb-resize,input,select,textarea,a,label") || (event.target.closest("button") && !event.target.closest(".owb-layout-open"))) return; const owner = event.target.closest?.("[data-owb-node]"); if (owner && owner !== element) return; pointerInteraction(event, node, element, "move"); };
