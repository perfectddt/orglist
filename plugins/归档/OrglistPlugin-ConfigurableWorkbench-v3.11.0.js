/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.11.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.11.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "四象限视图内置可自定义的重要/紧急判定；文字格式选项仅装饰节点显示" },
    activate(api) {
      ...
      function renderDrawer() {
        ...
        $("#owbSave").onclick = () => { const nodeKind = kind.value; const nodeContent = nodeKind === "window" ? content.value : ""; if (!["task", "button", "decor"].includes(nodeContent) && config.nodes.some((node) => node.content === nodeContent && node.id !== editingId)) { api.setNotice(`“${contentKinds[nodeContent].label}”只能添加一个。`); return; } if (!editingId && config.nodes.length >= 48) { api.setNotice("最多创建 48 个节点。"); return; }
          ...
          if (nodeKind === "window") { values.showContent = $("#owbShowContent").checked; if (nodeContent === "decor") { values.fontSize = Number($("#owbFontSize").value) > 0 ? clampFloat($("#owbFontSize").value, 6, 24, 0) : 0; values.zoom = Number($("#owbZoom").value) > 0 ? clampFloat(Number($("#owbZoom").value) / 100, 0.5, 2, 1) : 1; values.bold = $("#owbBold").checked; values.textColor = /^#[0-9a-f]{6}$/i.test($("#owbTextColor").value) ? $("#owbTextColor").value : ""; } }
