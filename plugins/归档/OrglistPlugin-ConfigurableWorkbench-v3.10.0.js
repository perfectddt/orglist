/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.10.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-export-history{padding:5px 7px;border:0;border-radius:6px;font-size:8px;color:var(--accent);background:var(--accentSoft)}
  ...
  function normalizeNode(value, index, ids) {
    ...
    const node = { id, kind, content, title: String(value?.title || (kind === "container" ? "容器" : contentKinds[content].label)).slice(0, 32), parent: String(value?.parent || "root"), x: clampFloat(value?.x, 0, 12 - w, 0), y: clampFloat(value?.y, 0, 240, 0), w, h: clampFloat(value?.h, 0.5, 24, kind === "container" ? 8 : 4), floating: !!value?.floating, locked: !!value?.locked, collapsible: !!value?.floating && !!value?.collapsible, autoCollapse: !!value?.floating && !!value?.collapsible && !!value?.autoCollapse, collapsed: !!value?.floating && !!value?.collapsible && !!value?.collapsed, cw: clamp(value?.cw, 1, 12, 2), ch: clamp(value?.ch, 1, 8, 1), cx: clampFloat(value?.cx, 0, 11.5, clampFloat(value?.x, 0, 11.5, 0)), cy: clampFloat(value?.cy, 0, 240, clampFloat(value?.y, 0, 240, 0)), showFrame: !!value?.showFrame, showLabel: value && Object.prototype.hasOwnProperty.call(value, "showLabel") ? !!value.showLabel : content === "task" };
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.10.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "修正四象限分类；中间主清单可添加多个，每个可应用当前筛选条件" },
    activate(api) {
      ...
      function taskImportant(task) { return task.priority === "A" || task.tags.includes("重要"); }
      function taskUrgent(task) { if (task.tags.includes("紧急")) return true; const actionDate = task.deadline || task.scheduled; return !!actionDate && actionDate <= addDays(api.today(), 3); }
      function quadrantMarkup(node, tasks) {
        const open = tasks.filter((task) => !task.done);
        const groups = [
          { key: "q1", title: "重要且紧急", items: open.filter((task) => taskImportant(task) && taskUrgent(task)) },
          { key: "q2", title: "重要不紧急", items: open.filter((task) => taskImportant(task) && !taskUrgent(task)) },
          { key: "q3", title: "紧急不重要", items: open.filter((task) => !taskImportant(task) && taskUrgent(task)) },
          { key: "q4", title: "不重要不紧急", items: open.filter((task) => !taskImportant(task) && !taskUrgent(task)) },
        ];
      ...
      function taskQueryOptions(node) {
        const calendar = isCalendar(node.view); const quadrant = node.view === "quadrant"; const wide = calendar || quadrant;
        const options = { view: wide ? "all" : node.view, query: node.query || "", limit: wide ? 200 : node.limit };
      ...
      function taskMarkup(node) {
        const tasks = api.queryTasks(taskQueryOptions(node));
      ...
      function nodeMarkup(node) {
        ...
        return `<section class="owb-node ${node.kind === "container" ? "owb-container" : "owb-window"} ${node.floating ? "floating" : "docked"} ${node.locked ? "locked" : ""} ${node.collapsed ? "collapsed" : ""} ${node.showFrame ? "owb-frame" : "owb-noframe"} ${node.showTitle ? "owb-show-title" : "owb-no-title"} ${node.showContent ? "" : "owb-hide-content"} ${node.bold ? "owb-bold" : ""} ${node.content === "smart" && node.vertical ? "owb-smart-vertical" : ""} ${node.kind === "window" ? `owb-core-${node.content}` : ""}" data-owb-node="${api.escapeHtml(node.id)}" style="${node.fontSize ? `--owb-font-size:${node.fontSize}px;` : ""}${node.zoom && node.zoom !== 1 ? `--owb-zoom:${node.zoom};` : ""}${node.textColor ? `--owb-text-color:${node.textColor};` : ""}"><div class="owb-node-inner"><header class="owb-node-head"><strong>${api.escapeHtml(node.title)}</strong></header><div class="owb-node-body">${body}</div></div><span class="owb-resize"></span></section>`;
      }
      ...
      function pointerInteraction(event, node, element, mode) {
        ...
        const move = (next) => { if (next.pointerId !== event.pointerId) return; const dx = Math.round((next.clientX - start.px) / columnStep / step) * step; const dy = Math.round((next.clientY - start.py) / rowStep / step) * step; const candidate = { x: start.x, y: start.y, w: start.w, h: start.h };
          ...
          else { candidate.w = Math.max(0.5, Math.min(12 - candidate.x, start.w + dx)); candidate.h = Math.max(0.5, Math.min(24, start.h + dy)); }
      ...
      function renderWorkspace() {
        ...
        canvas.querySelectorAll("[data-apply-filters]").forEach((button) => button.onclick = () => {
          ...
          node.filters = { status, priority, tag, search };
      ...
      function renderDrawer() {
        ...
        $(".owb-export-history").onclick = exportHistory;
        ...
        sync(); kind.onchange = sync; content.onchange = () => { if (!$("#owbTitle").value.trim()) $("#owbTitle").value = contentKinds[content.value].label; sync(); }; floating.onchange = sync; collapsible.onchange = sync;
        ...
        $("#owbSave").onclick = () => { const nodeKind = kind.value; const nodeContent = nodeKind === "window" ? content.value : ""; if (!["task", "button", "decor"].includes(nodeContent) && config.nodes.some((node) => node.content === nodeContent && node.id !== editingId)) { api.setNotice(`“${contentKinds[nodeContent].label}”只能添加一个。`); return; } if (!editingId && config.nodes.length >= 48) { api.setNotice("最多创建 48 个节点。"); return; }
          ...
          if (nodeKind === "window") { values.showContent = $("#owbShowContent").checked; values.fontSize = Number($("#owbFontSize").value) > 0 ? clampFloat($("#owbFontSize").value, 6, 24, 0) : 0; values.zoom = Number($("#owbZoom").value) > 0 ? clampFloat(Number($("#owbZoom").value) / 100, 0.5, 2, 1) : 1; values.bold = $("#owbBold").checked; values.textColor = /^#[0-9a-f]{6}$/i.test($("#owbTextColor").value) ? $("#owbTextColor").value : ""; }
