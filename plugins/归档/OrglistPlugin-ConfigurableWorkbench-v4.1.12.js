/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"4.1.12","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-node{position:relative;display:flex;min-width:0;min-height:0;overflow:hidden;background:var(--panel);border:1px solid var(--line);border-radius:12px;box-shadow:var(--shadow)}.owb-node.floating{position:absolute;z-index:45;box-shadow:0 22px 60px rgba(0,0,0,.22)}.owb-node.dragging,.owb-node.resizing{z-index:60!important;border-color:var(--accent);opacity:.93}.owb-node.locked{border-color:color-mix(in srgb,var(--accent) 32%,var(--line))}
    ...
    .owb-core-layout{min-width:64px;min-height:48px}.owb-layout-open{min-width:52px;min-height:40px;font-size:11px;padding:9px 30px 9px 10px}
  `;
  ...
  function normalizeNode(value, index, ids) {
    ...
    node.layer = clamp(value?.layer, 0, 50, 0);
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "4.1.12", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "修复布局按钮重建后重复 click 导致模式立即切回" },
    activate(api) {
      let config = normalizeConfig(api.getSetting("config", defaults));
      let editingId = null; let lastSummary = { selected: "", detailOpen: false, view: "" }; let suppressNextLayoutClick = false; let layoutClickGuardTimer = 0;
      ...
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "4.1.12", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function geometry(element, node) {
        ...
        element.style.gridColumn = ""; element.style.gridRow = ""; element.style.position = "absolute"; element.style.left = ""; element.style.top = ""; element.style.width = ""; element.style.height = ""; element.style.zIndex = node.content === "layout" ? "55" : String(node.layer + 1);
      ...
      function pointerInteraction(event, node, element, mode) {
        ...
        hideTooltip();
        const parent = element.parentElement; const nested = parent.classList.contains("owb-container-body"); const rect = parent.getBoundingClientRect(); const gap = nested ? 8 : 10; const row = nested ? 48 : 64; const columnStep = rect.width / 12; const rowStep = row + gap; const step = event.shiftKey ? 1 : 0.5; const startRect = nodeRect(node); const start = { px: event.clientX, py: event.clientY, x: startRect.x, y: startRect.y, w: startRect.w, h: startRect.h }; const activationDistance = mode === "move" && node.content === "layout" ? (event.pointerType === "touch" ? 12 : 4) : 0; let changed = false;
        element.classList.add(mode === "move" ? "dragging" : "resizing");
        const move = (next) => { if (next.pointerId !== event.pointerId) return; const pixelX = next.clientX - start.px; const pixelY = next.clientY - start.py; if (activationDistance && Math.hypot(pixelX, pixelY) < activationDistance) return; const dx = Math.round(pixelX / columnStep / step) * step; const dy = Math.round(pixelY / rowStep / step) * step; const candidate = { x: start.x, y: start.y, w: start.w, h: start.h };
      ...
      function bindNode(element, node) {
        ...
        element.onpointerdown = (event) => { if (event.target.closest(".owb-resize,button,input,select,textarea,a,label")) return; if (node.content === "layout" && !event.target.closest(".owb-layout-drag")) return; const owner = event.target.closest?.("[data-owb-node]"); if (owner && owner !== element) return; pointerInteraction(event, node, element, "move"); };
        ...
        const layoutButton = element.querySelector(".owb-layout-open");
        let layoutPress = null;
        const activateLayoutButton = () => { const wasLayout = config.mode === "layout"; config.mode = wasLayout ? "use" : "layout"; if (wasLayout) pushHistory(); editingId = null; drawer.classList.toggle("show", config.mode === "layout"); save(); renderWorkspace(); renderDrawer(); };
        layoutButton?.addEventListener("pointerdown", (event) => { if (event.isPrimary === false || (event.button !== undefined && event.button !== 0)) return; layoutPress = { id: event.pointerId, x: event.clientX, y: event.clientY }; });
        layoutButton?.addEventListener("pointerup", (event) => { if (!layoutPress || layoutPress.id !== event.pointerId) return; const distance = Math.hypot(event.clientX - layoutPress.x, event.clientY - layoutPress.y); layoutPress = null; if (distance > 18) return; event.preventDefault(); event.stopPropagation(); suppressNextLayoutClick = true; window.clearTimeout(layoutClickGuardTimer); layoutClickGuardTimer = window.setTimeout(() => { suppressNextLayoutClick = false; }, 500); activateLayoutButton(); });
        layoutButton?.addEventListener("pointercancel", () => { layoutPress = null; });
        layoutButton?.addEventListener("click", (event) => { event.stopPropagation(); if (suppressNextLayoutClick) { suppressNextLayoutClick = false; window.clearTimeout(layoutClickGuardTimer); return; } activateLayoutButton(); });
      ...
      function nodeDescription(node) { const rect = nodeRect(node); const position = `${node.floating ? "浮动" : "停靠"} · 层级 ${node.layer} · X${formatNum(rect.x + 1)} Y${formatNum(rect.y + 1)} · ${formatNum(rect.w)}×${formatNum(rect.h)}${node.visible ? "" : " · 已隐藏"}${node.collapsed ? " · 已折叠" : ""}`; const label = node.content === "task" ? `${viewLabels[node.view]}${node.query ? ` · ${node.query}` : ""} · ` : ""; return node.kind === "container" ? position : `${label}${position}`; }
      ...
      function renderDrawer() {
        ...
        drawer.querySelectorAll("[data-layer-action]").forEach((button) => button.onclick = () => { const current = clamp($("#owbLayer").value, 0, 50, 0); const action = button.dataset.layerAction; $("#owbLayer").value = String(action === "top" ? 50 : action === "bottom" ? 0 : action === "up" ? Math.min(50, current + 1) : Math.max(0, current - 1)); });
        ...
        drawer.querySelectorAll(".owb-node-config[data-locate]").forEach((row) => row.onclick = (event) => { if (event.target.closest("button,input,label")) return; locateNode(row.dataset.locate); });
        ...
        drawer.querySelectorAll("[data-hide]").forEach((checkbox) => { checkbox.onclick = (event) => event.stopPropagation(); checkbox.onchange = (event) => { event.stopPropagation(); const node = findNode(checkbox.dataset.hide); if (!node || node.content === "layout") return; node.visible = !checkbox.checked; save(); renderWorkspace(); renderDrawer(); }; });
      ...
      return { deactivate() { clearTimeout(locateTimer); clearTimeout(layoutClickGuardTimer); clearInterval(countdownTimer); document.removeEventListener("pointerdown", outsideCollapse); restoreRegions(); document.body.classList.remove("owb-blank"); } };
    },
  });
})();
