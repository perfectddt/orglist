/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.22.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-mode-use .owb-node.owb-label:not(.collapsed)>.owb-node-inner>.owb-node-head{display:none}.owb-button-body{display:grid;place-items:center;height:100%;padding:6px}.owb-node-button{width:100%;height:100%;padding:8px 10px;color:#fff;background:var(--accent);border:0;border-radius:8px;font-size:10px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  ...
  function normalizeNode(value, index, ids) {
    ...
    const node = { id, kind, content, title: String(value?.title || (kind === "container" ? "容器" : contentKinds[content].label)).slice(0, 32), parent: String(value?.parent || "root"), x: clampFloat(value?.x, 0, 12 - w, 0), y: clampFloat(value?.y, 0, 240, 0), w, h: clampFloat(value?.h, 0.5, 24, kind === "container" ? 8 : 4), floating: !!value?.floating, locked: !!value?.locked, collapsible: !!value?.floating && !!value?.collapsible, autoCollapse: !!value?.floating && !!value?.collapsible && !!value?.autoCollapse, collapsed: !!value?.floating && !!value?.collapsible && !!value?.collapsed, cw: clampFloat(value?.cw, 0.1, 12, 2), ch: clampFloat(value?.ch, 0.1, 15, 1), cx: clampFloat(value?.cx, 0, 11.5, clampFloat(value?.x, 0, 11.5, 0)), cy: clampFloat(value?.cy, 0, 240, clampFloat(value?.y, 0, 240, 0)), showFrame: !!value?.showFrame, showLabel: value && Object.prototype.hasOwnProperty.call(value, "showLabel") ? !!value.showLabel : content === "task" };
    ...
    if (content === "button") { node.text = String(value?.text || "按钮").slice(0, 24); node.target = String(value?.target || "").slice(0, 64); }
    ...
    if (content === "task" || content === "mainList") { node.view = Object.prototype.hasOwnProperty.call(viewLabels, value?.view) ? value.view : "all"; node.query = String(value?.query || "").slice(0, 180); node.limit = clamp(value?.limit, 1, 200, 20); node.calendarCursor = validDate(value?.calendarCursor) ? value.calendarCursor : ""; node.statisticsPeriod = ["week", "month", "year"].includes(value?.statisticsPeriod) ? value.statisticsPeriod : "week"; node.filters = value?.filters && typeof value.filters === "object" ? value.filters : {}; node.filterButton = !!value?.filterButton; node.quadrantRules = { important: ["A", "AB", "ANY", "TAG"].includes(value?.quadrantRules?.important) ? value.quadrantRules.important : "A", urgent: ["0", "3", "7", "14", "OVERDUE", "TAG"].includes(value?.quadrantRules?.urgent) ? value.quadrantRules.urgent : "3" }; node.openRule = ["done", "status", "closed"].includes(value?.openRule) ? value.openRule : "done"; }
    return node;
  }
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.22.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "通过宿主配置注册表动态编辑主程序公开设置" },
    activate(api) {
      ...
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "3.5.0", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function statisticsMarkup(node) {
        ...
        const max = Math.max(1, ...entries.map((item) => item.count)), files = new Map(); completed.forEach((task) => files.set(task.fileName, (files.get(task.fileName) || 0) + 1)); const categories = [...files].sort((a, b) => b[1] - a[1]).slice(0, 8), categoryMax = Math.max(1, ...categories.map((item) => item[1]));
        return `<div class="owb-statistics"><div class="owb-stat-tools"><strong>${bounds.start} — ${bounds.end}</strong>${[["week","周"],["month","月"],["year","年"]].map(([value,label]) => `<button data-stat-period="${value}" class="${period === value ? "active" : ""}">${label}</button>`).join("")}</div><div class="owb-stat-grid"><div class="owb-stat-card"><span>${completed.length}</span><small>完成任务</small></div><div class="owb-stat-card"><span>${planned.length ? Math.round(plannedDone.length / planned.length * 100) : 0}%</span><small>计划完成率 · 现存计划口径</small></div><div class="owb-stat-card"><span>${dated.length ? Math.round(onTime.length / dated.length * 100) : 0}%</span><small>按期完成率</small></div><div class="owb-stat-card"><span>${overdue.length}</span><small>当前逾期</small></div></div><section class="owb-stat-section"><h4>完成趋势</h4><div class="owb-stat-trend">${entries.map((item) => `<span class="owb-stat-column"><span class="owb-stat-number">${item.count || ""}</span><span class="owb-stat-bar" style="height:${Math.max(2, item.count / max * 70)}px"></span><span class="owb-stat-label">${item.label}</span></span>`).join("")}</div></section><section class="owb-stat-section"><h4>按文件分类</h4><div class="owb-stat-categories">${categories.map(([name,count]) => `<div class="owb-stat-category"><span>${api.escapeHtml(name)}</span><span class="owb-stat-track"><span class="owb-stat-fill" style="width:${count / categoryMax * 100}%"></span></span><span>${count}</span></div>`).join("") || '<div class="owb-empty">当前周期没有完成记录</div>'}</div><div class="owb-stat-note">取消任务不计入完成数；统计窗口读取完整任务数据，不受普通窗口“最多显示”限制。</div></section></div>`;
      }
      ...
      function taskQueryOptions(node, view) {
        ...
        const queryView = wide || (view === "open" && node.openRule !== "done") ? "all" : view;
      ...
      function bindNode(element, node) {
        ...
        if (node.content === "task" && node.view === "statistics") element.querySelectorAll("[data-stat-period]").forEach((button) => button.onclick = () => { node.statisticsPeriod = button.dataset.statPeriod; save(); renderWorkspace(); });
      }
      ...
      function renderWorkspace() {
        ...
        canvas.querySelectorAll("[data-owb-target]").forEach((button) => button.onclick = () => revealTarget(button.dataset.owbTarget));
        canvas.querySelectorAll("[data-apply-filters]").forEach((button) => button.onclick = () => {
          ...
          const search = String(document.querySelector("#search")?.value || "").trim();
          let appView = document.querySelector("#nav [data-view].active")?.dataset?.view || lastSummary.view || "";
          ...
          const view = node.content === "mainList" ? appView : node.view;
          const fileId = document.querySelector("#files [data-file].active")?.dataset?.file || "";
          let customQuery = "";
          const activeCustom = document.querySelector("#customLists .customItem.active")?.dataset?.customItem;
          if (activeCustom) { try { const customs = JSON.parse(localStorage.getItem("org-task-custom-lists-v1") || "[]"); customQuery = String((customs.find((item) => String(item.id) === String(activeCustom)) || {}).query || "").trim(); } catch (_) {} }
          node.filters = { status, priority, tag, search, view, fileId, customQuery };
          if (node.content === "mainList" && viewLabels[view]) node.view = view;
          save(); renderWorkspace(); api.setNotice("已把当前筛选应用到该主清单。");
        });
        syncCanvasSize();
      }
      ...
      function nodeDescription(node) { const rect = nodeRect(node); const position = `${node.floating ? "浮动" : "停靠"} · X${formatNum(rect.x + 1)} Y${formatNum(rect.y + 1)} · ${formatNum(rect.w)}×${formatNum(rect.h)}${node.collapsed ? " · 已折叠" : ""}`; const label = node.content === "task" ? `${viewLabels[node.view]}${node.query ? ` · ${node.query}` : ""} · ` : ""; return node.kind === "container" ? position : `${label}${position}`; }
      ...
      function revealTarget(id) {
        ...
        if (target.collapsed) { target.collapsed = false; save(); renderWorkspace(); }
        locateNode(id);
      }
      ...
      function renderDrawer() {
        ...
        drawer.innerHTML = `<div class="owb-config-head"><h3>空白画布布局</h3><span class="owb-config-actions"><button class="owb-reset" type="button">恢复原始布局</button><button class="owb-use-mode" type="button">使用模式</button><button class="owb-config-close" aria-label="关闭布局设置">×</button></span></div><button class="owb-history-toggle" type="button">默认布局与历史 <span class="owb-caret">▸</span></button><div class="owb-history-box" hidden><div class="owb-section-row"><h4>默认布局与历史</h4><span class="owb-config-actions"><button class="owb-set-default" type="button">设为默认布局</button><button class="owb-export-history" type="button">导出</button><button class="owb-import-history" type="button">导入</button><button class="owb-clear-history" type="button">清空历史</button></span></div><div class="owb-history">${historyItems}</div></div><button class="owb-history-toggle owb-app-settings-toggle" type="button">主程序公开设置 <span class="owb-caret">▸</span></button><div class="owb-app-settings-box" hidden>${appSettingsMarkup()}</div><p class="owb-hint">当前是布局模式：拖动或缩放未锁定节点（默认按半格微调，按住 Shift 吸附整格；从节点任意处拖动），双击任意节点直接打开它的编辑表单；鼠标划过节点会弹出注释（仅布局模式），点击下方节点行可在画布中定位高亮；面板标题栏可拖动位置。主程序公开设置由宿主动态提供，新增公开配置不需要再修改本插件。完成后点击“使用模式”，页面只保留真正的功能内容。</p><div class="owb-section-row"><h4>节点</h4><button class="owb-auto" type="button">自动排列未锁定停靠项</button></div><div>${list}</div>
          ...
          <span class="owb-button-fields"><label class="owb-field wide"><span>按钮文字</span><input id="owbButtonText" maxlength="24"></label><label class="owb-field wide"><span>点击显示目标</span><select id="owbButtonTarget"><option value="">（无）</option>${config.nodes.filter((node) => node.id !== active?.id).map((node) => `<option value="${api.escapeHtml(node.id)}">${api.escapeHtml(node.title)}</option>`).join("")}</select></label></span>
        ...
        sync(); kind.onchange = sync; $("#owbView").onchange = sync; content.onchange = () => { if (!$("#owbTitle").value.trim()) $("#owbTitle").value = contentKinds[content.value].label; sync(); }; floating.onchange = sync; collapsible.onchange = sync;
        ...
        $("#owbSave").onclick = () => { const nodeKind = kind.value; const nodeContent = nodeKind === "window" ? content.value : ""; if (!["task", "button", "decor", "mainList"].includes(nodeContent) && config.nodes.some((node) => node.content === nodeContent && node.id !== editingId)) { api.setNotice(`“${contentKinds[nodeContent].label}”只能添加一个。`); return; } if (!editingId && config.nodes.length >= 48) { api.setNotice("最多创建 48 个节点。"); return; }
          ...
          if (nodeKind === "window") { values.showContent = $("#owbShowContent").checked; values.zoom = Number($("#owbZoom").value) > 0 ? clampFloat(Number($("#owbZoom").value) / 100, 0.5, 2, 1) : 1; if (nodeContent === "decor") { values.fontSize = Number($("#owbFontSize").value) > 0 ? clampFloat($("#owbFontSize").value, 6, 24, 0) : 0; values.bold = $("#owbBold").checked; values.textColor = /^#[0-9a-f]{6}$/i.test($("#owbTextColor").value) ? $("#owbTextColor").value : ""; } }
