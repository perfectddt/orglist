/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.2.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.2.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "布局模式按半格微调、双击编辑节点并支持定位高亮；使用模式可逐节点保留框线与标题" },
    activate(api) {
      ...
      function nodeDescription(node) { const num = (value) => Number.isInteger(value) ? String(value) : value.toFixed(1); const position = `${node.floating ? "浮动" : "停靠"} · X${num(node.x + 1)} Y${num(node.y + 1)} · ${num(node.w)}×${num(node.h)}`; return node.kind === "container" ? position : `${contentKinds[node.content]?.label || node.content}${node.content === "task" ? ` · ${viewLabels[node.view]}${node.query ? ` · ${node.query}` : ""}` : ""} · ${position}`; }
