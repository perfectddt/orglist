/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.1.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-root{flex:1;min-width:0;height:100vh;padding:10px;overflow:auto}.owb-canvas,.owb-container-body{position:relative;display:grid;grid-template-columns:repeat(12,minmax(0,1fr));align-items:stretch}.owb-canvas{grid-auto-rows:64px;gap:10px;min-height:100%}.owb-container-body{grid-auto-rows:48px;gap:8px;flex:1;min-height:0;padding:7px;overflow:auto;background:color-mix(in srgb,var(--soft) 55%,transparent)}
  ...
  function normalizeNode(value, index, ids) {
    ...
    const w = clamp(value?.w, 1, 12, kind === "container" ? 6 : 4);
    const node = { id, kind, content, title: String(value?.title || (kind === "container" ? "容器" : contentKinds[content].label)).slice(0, 32), parent: String(value?.parent || "root"), x: clamp(value?.x, 0, 12 - w, 0), y: clamp(value?.y, 0, 240, 0), w, h: clamp(value?.h, 1, 24, kind === "container" ? 8 : 4), floating: !!value?.floating, locked: !!value?.locked, collapsible: !!value?.floating && !!value?.collapsible, autoCollapse: !!value?.floating && !!value?.collapsible && !!value?.autoCollapse, collapsed: !!value?.floating && !!value?.collapsible && !!value?.collapsed, cw: clamp(value?.cw, 1, 12, 2), ch: clamp(value?.ch, 1, 8, 1) };
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.1.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "使用模式只显示功能，布局模式双击编辑容器与窗口" },
    activate(api) {
      ...
      function nodeMarkup(node) {
        ...
        return `<section class="owb-node ${node.kind === "container" ? "owb-container" : "owb-window"} ${node.floating ? "floating" : "docked"} ${node.locked ? "locked" : ""} ${node.collapsed ? "collapsed" : ""} ${node.kind === "window" ? `owb-core-${node.content}` : ""}" data-owb-node="${api.escapeHtml(node.id)}"><div class="owb-node-inner"><header class="owb-node-head"><strong>${api.escapeHtml(node.title)}</strong><span class="owb-node-kind">${api.escapeHtml(nodeLabel(node))}</span></header><div class="owb-node-body">${body}</div></div><span class="owb-resize"></span></section>`;
      }

      function geometry(element, node) {
        const size = nodeSize(node); const nested = element.parentElement?.classList.contains("owb-container-body"); const row = nested ? 48 : 64; const gap = nested ? 8 : 10;
        element.style.gridColumn = ""; element.style.gridRow = ""; element.style.left = ""; element.style.top = ""; element.style.width = ""; element.style.height = "";
        ...
        else { element.style.gridColumn = `${node.x + 1} / span ${size.w}`; element.style.gridRow = `${node.y + 1} / span ${size.h}`; }
      }
      ...
      function pointerInteraction(event, node, element, mode) {
        if (config.mode !== "layout" || node.locked || window.matchMedia?.("(max-width:760px)").matches) return;
        if (event.button !== undefined && event.button !== 0) return; event.preventDefault();
        const parent = element.parentElement; const nested = parent.classList.contains("owb-container-body"); const rect = parent.getBoundingClientRect(); const gap = nested ? 8 : 10; const row = nested ? 48 : 64; const columnStep = rect.width / 12; const rowStep = row + gap; const startSize = nodeSize(node); const start = { px: event.clientX, py: event.clientY, x: node.x, y: node.y, w: startSize.w, h: startSize.h }; let changed = false;
        element.classList.add(mode === "move" ? "dragging" : "resizing");
        const move = (next) => { if (next.pointerId !== event.pointerId) return; const dx = Math.round((next.clientX - start.px) / columnStep); const dy = Math.round((next.clientY - start.py) / rowStep); const candidate = { x: node.x, y: node.y, w: start.w, h: start.h };
          if (mode === "move") { candidate.x = Math.max(0, Math.min(12 - start.w, start.x + dx)); candidate.y = Math.max(0, Math.min(240, start.y + dy)); }
          else { candidate.w = Math.max(1, Math.min(12 - node.x, start.w + dx)); candidate.h = Math.max(1, Math.min(24, start.h + dy)); }
          if (!collision(candidate, node)) { node.x = candidate.x; node.y = candidate.y; if (node.floating && node.collapsible && node.collapsed) { node.cw = candidate.w; node.ch = candidate.h; } else { node.w = candidate.w; node.h = candidate.h; } geometry(element, node); changed = true; }
        };
        const finish = (next) => { if (next.pointerId !== event.pointerId) return; window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", finish); window.removeEventListener("pointercancel", finish); element.classList.remove("dragging", "resizing"); if (changed) { save(); if (drawer.classList.contains("show")) renderDrawer(); } };
      ...
      function bindNode(element, node) {
        ...
        header.onpointerdown = (event) => { if (config.mode === "use" && node.collapsed) { node.collapsed = false; save(); renderWorkspace(); return; } pointerInteraction(event, node, element, "move"); };
        ...
        element.querySelector(".owb-layout-open")?.addEventListener("click", () => { config.mode = config.mode === "layout" ? "use" : "layout"; editingId = null; drawer.classList.toggle("show", config.mode === "layout"); save(); renderWorkspace(); renderDrawer(); });
        if (node.content === "task" && isCalendar(node.view)) bindCalendar(element, node);
      }

      function renderWorkspace() {
        restoreRegions(); canvas.className = `owb-canvas owb-mode-${config.mode}`; canvas.innerHTML = childNodes("root").map(nodeMarkup).join("");
        config.nodes.forEach((node) => { const element = [...canvas.querySelectorAll("[data-owb-node]")].find((item) => item.dataset.owbNode === node.id); if (!element) return; bindNode(element, node);
          ...
          if (node.content === "detail") element.hidden = !lastSummary.detailOpen;
        });
      ...
      function nodeDescription(node) { const position = `${node.floating ? "浮动" : "停靠"} · X${node.x + 1} Y${node.y + 1} · ${node.w}×${node.h}`; return node.kind === "container" ? position : `${contentKinds[node.content]?.label || node.content}${node.content === "task" ? ` · ${viewLabels[node.view]}${node.query ? ` · ${node.query}` : ""}` : ""} · ${position}`; }
      ...
      function renderDrawer() {
        ...
        drawer.innerHTML = `<div class="owb-config-head"><h3>空白画布布局</h3><span class="owb-config-actions"><button class="owb-reset" type="button">恢复原始布局</button><button class="owb-use-mode" type="button">使用模式</button><button class="owb-config-close" aria-label="关闭布局设置">×</button></span></div><p class="owb-hint">当前是布局模式：拖动或缩放未锁定节点，双击任意节点即可直接打开它的编辑表单。锁定、浮动和折叠都在表单中设置；完成后点击“使用模式”，页面只保留真正的功能内容。</p><div class="owb-section-row"><h4>节点</h4><button class="owb-auto" type="button">自动排列未锁定停靠项</button></div><div>${list}</div>
          ...
          <label class="owb-field"><span>X（1–12）</span><input id="owbX" type="number" min="1" max="12"></label><label class="owb-field"><span>Y（1 起）</span><input id="owbY" type="number" min="1" max="241"></label><label class="owb-field"><span>展开宽度（1–12）</span><input id="owbW" type="number" min="1" max="12"></label><label class="owb-field"><span>展开高度</span><input id="owbH" type="number" min="1" max="24"></label>
          <div class="owb-checks"><label class="owb-check"><input id="owbFloating" type="checkbox">浮动（允许重叠）</label><label class="owb-check"><input id="owbLocked" type="checkbox">锁定位置</label><label class="owb-check"><input id="owbCollapsible" type="checkbox">启用折叠尺寸</label><label class="owb-check"><input id="owbAutoCollapse" type="checkbox">点击外部自动折叠</label></div><span class="owb-float-fields"><label class="owb-field"><span>折叠宽度</span><input id="owbCW" type="number" min="1" max="12"></label><label class="owb-field"><span>折叠高度</span><input id="owbCH" type="number" min="1" max="8"></label></span>
        ...
        $("#owbSave").onclick = () => { const nodeKind = kind.value; const nodeContent = nodeKind === "window" ? content.value : ""; if (nodeContent !== "task" && config.nodes.some((node) => node.content === nodeContent && node.id !== editingId)) { api.setNotice(`“${contentKinds[nodeContent].label}”只能添加一个。`); return; } if (!editingId && config.nodes.length >= 48) { api.setNotice("最多创建 48 个节点。"); return; }
          const existing = findNode(editingId); const parent = nodeKind === "container" ? "root" : $("#owbParent").value; const values = { kind: nodeKind, content: nodeContent, title: ($("#owbTitle").value.trim() || (nodeKind === "container" ? "容器" : contentKinds[nodeContent].label)).slice(0, 32), parent, x: clamp(Number($("#owbX").value) - 1, 0, 11, 0), y: clamp(Number($("#owbY").value) - 1, 0, 240, 0), w: clamp($("#owbW").value, 1, 12, 4), h: clamp($("#owbH").value, 1, 24, 4), floating: floating.checked, locked: $("#owbLocked").checked, collapsible: floating.checked && collapsible.checked, autoCollapse: floating.checked && collapsible.checked && $("#owbAutoCollapse").checked, cw: clamp($("#owbCW").value, 1, 12, 2), ch: clamp($("#owbCH").value, 1, 8, 1) };
      ...
      const outsideCollapse = (event) => { if (config.mode !== "use") return; const owner = event.target.closest?.("[data-owb-node]")?.dataset?.owbNode; const closing = config.nodes.filter((node) => node.floating && node.collapsible && node.autoCollapse && !node.collapsed && node.id !== owner); if (closing.length) { closing.forEach((node) => { node.collapsed = true; const element = [...canvas.querySelectorAll("[data-owb-node]")].find((item) => item.dataset.owbNode === node.id); if (element) { element.classList.add("collapsed"); geometry(element, node); } }); save(); if (drawer.classList.contains("show")) renderDrawer(); } };
      ...
      return { deactivate() { document.removeEventListener("pointerdown", outsideCollapse); restoreRegions(); document.body.classList.remove("owb-blank"); } };
    },
  });
})();
