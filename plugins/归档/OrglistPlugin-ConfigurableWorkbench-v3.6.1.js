/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.6.1","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.6.1", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "布局模式保持使用模式的折叠状态且展开/折叠点击失效，可自由调整折叠样子；折叠前后位置大小各自独立" },
    activate(api) {
      ...
      function renderDrawer() {
        ...
        drawer.innerHTML = `<div class="owb-config-head"><h3>空白画布布局</h3><span class="owb-config-actions"><button class="owb-reset" type="button">恢复原始布局</button><button class="owb-use-mode" type="button">使用模式</button><button class="owb-config-close" aria-label="关闭布局设置">×</button></span></div><button class="owb-history-toggle" type="button">默认布局与历史 <span class="owb-caret">▸</span></button><div class="owb-history-box" hidden><div class="owb-section-row"><h4>默认布局与历史</h4><span class="owb-config-actions"><button class="owb-set-default" type="button">设为默认布局</button><button class="owb-export-history" type="button">导出</button><button class="owb-clear-history" type="button">清空历史</button></span></div><div class="owb-history">${historyItems}</div></div><p class="owb-hint">当前是布局模式：拖动或缩放未锁定节点（默认按半格微调，按住 Shift 吸附整格；从节点任意处拖动），双击任意节点直接打开它的编辑表单；鼠标划过节点会弹出注释（仅布局模式），点击下方节点行可在画布中定位高亮；面板标题栏可拖动位置。表单中可为每个节点选择使用模式是否保留框线；按钮节点、装饰文字/图案和智能清单一列列横排都在表单里设置。完成后点击“使用模式”，页面只保留真正的功能内容。</p><div class="owb-section-row"><h4>节点</h4><button class="owb-auto" type="button">自动排列未锁定停靠项</button></div><div>${list}</div>
          ...
          <span class="owb-decor-fields"><label class="owb-field"><span>图案类型</span><select id="owbDecorKind"><option value="text">文字</option><option value="line-h">横线</option><option value="line-v">竖线</option><option value="dots">圆点</option><option value="blank">空白</option></select></label><label class="owb-field"><span>显示文字</span><input id="owbDecorText" maxlength="48"></label></span>
