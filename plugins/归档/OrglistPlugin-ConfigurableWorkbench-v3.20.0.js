/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.20.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  function normalizeNode(value, index, ids) {
    ...
    if (content === "task" || content === "mainList") { node.view = Object.prototype.hasOwnProperty.call(viewLabels, value?.view) ? value.view : "all"; node.query = String(value?.query || "").slice(0, 180); node.limit = clamp(value?.limit, 1, 200, 20); node.calendarCursor = validDate(value?.calendarCursor) ? value.calendarCursor : ""; node.filters = value?.filters && typeof value.filters === "object" ? value.filters : {}; node.filterButton = !!value?.filterButton; node.quadrantRules = { important: ["A", "AB", "ANY", "TAG"].includes(value?.quadrantRules?.important) ? value.quadrantRules.important : "A", urgent: ["0", "3", "7", "14", "OVERDUE", "TAG"].includes(value?.quadrantRules?.urgent) ? value.quadrantRules.urgent : "3" }; node.openRule = ["done", "status", "closed"].includes(value?.openRule) ? value.openRule : "done"; }
    return node;
  }
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.20.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "新增“未完成的定义”可配置筛选规则；详情按选中自动展开" },
    activate(api) {
      ...
      function pointerInteraction(event, node, element, mode) {
        ...
        const move = (next) => { if (next.pointerId !== event.pointerId) return; const dx = Math.round((next.clientX - start.px) / columnStep / step) * step; const dy = Math.round((next.clientY - start.py) / rowStep / step) * step; const candidate = { x: start.x, y: start.y, w: start.w, h: start.h };
          ...
          else { const minStep = node.floating && node.collapsible && node.collapsed ? 0.1 : 0.5; const maxH = node.floating && node.collapsible && node.collapsed ? 8 : 24; candidate.w = Math.max(minStep, Math.min(12 - candidate.x, start.w + dx)); candidate.h = Math.max(minStep, Math.min(maxH, start.h + dy)); }
