/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.7.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.7.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "逐节点控制使用模式是否显示内容、文字大小/加粗/缩放；任务窗口新增四象限视图；修复日历日期数字按钮样式" },
    activate(api) {
      ...
      function renderDrawer() {
        ...
        $("#owbSave").onclick = () => { const nodeKind = kind.value; const nodeContent = nodeKind === "window" ? content.value : ""; if (!["task", "button", "decor"].includes(nodeContent) && config.nodes.some((node) => node.content === nodeContent && node.id !== editingId)) { api.setNotice(`“${contentKinds[nodeContent].label}”只能添加一个。`); return; } if (!editingId && config.nodes.length >= 48) { api.setNotice("最多创建 48 个节点。"); return; }
          ...
          if (nodeContent === "decor") { values.decor = $("#owbDecorKind").value; values.text = $("#owbDecorText").value.slice(0, 48); }
          if (nodeContent === "smart") { values.vertical = $("#owbVertical").checked; }
          if (nodeKind === "window") { values.showContent = $("#owbShowContent").checked; values.fontSize = Number($("#owbFontSize").value) > 0 ? clampFloat($("#owbFontSize").value, 6, 24, 0) : 0; values.zoom = Number($("#owbZoom").value) > 0 ? clampFloat(Number($("#owbZoom").value) / 100, 0.5, 2, 1) : 1; values.bold = $("#owbBold").checked; }
