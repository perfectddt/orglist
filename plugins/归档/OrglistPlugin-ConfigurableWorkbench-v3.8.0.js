/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.8.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-node-body{color:var(--owb-text-color, inherit)}.owb-node .owb-node-body .owb-task-title,.owb-node .owb-node-body .owb-decor,.owb-node .owb-node-body .owb-empty,.owb-node .owb-node-body .owb-cal-task{color:var(--owb-text-color, inherit)}
  `;
  ...
  function normalizeConfig(value) {
    ...
    const nodes = source.slice(0, 48).map((node, index) => normalizeNode(node, index, ids)).filter((node) => {
      if (node.kind === "container" || ["task", "button", "decor"].includes(node.content)) return true;
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.8.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "装饰文字自动用作节点标题；新增文字颜色；任务/日历窗口按主清单样式着色（四象限红蓝黄绿）" },
    activate(api) {
      ...
      function taskImportant(task) { return task.priority === "A"; }
      function taskUrgent(task) { const actionDate = task.deadline || task.scheduled; return !!actionDate && actionDate <= addDays(api.today(), 7); }
      ...
      function taskMarkup(node) {
        ...
        if (node.view === "quadrant") return quadrantMarkup(node, tasks);
      ...
      function nodeMarkup(node) {
        ...
        const body = node.kind === "container" ? `<div class="owb-container-body">${children}</div>` : node.content === "task" ? taskMarkup(node) : node.content === "layout" ? `<div class="owb-layout-body"><button class="owb-layout-open" type="button">${config.mode === "layout" ? "使用模式" : "布局"}</button></div>` : node.content === "button" ? `<div class="owb-button-body"><button class="owb-node-button" data-owb-target="${api.escapeHtml(node.target || node.id)}">${api.escapeHtml(node.text || "按钮")}</button></div>` : node.content === "decor" ? decorMarkup(node) : "";
        return `<section class="owb-node ${node.kind === "container" ? "owb-container" : "owb-window"} ${node.floating ? "floating" : "docked"} ${node.locked ? "locked" : ""} ${node.collapsed ? "collapsed" : ""} ${node.showFrame ? "owb-frame" : "owb-noframe"} ${node.showLabel ? "owb-label" : "owb-nolabel"} ${node.showContent ? "" : "owb-hide-content"} ${node.bold ? "owb-bold" : ""} ${node.content === "smart" && node.vertical ? "owb-smart-vertical" : ""} ${node.kind === "window" ? `owb-core-${node.content}` : ""}" data-owb-node="${api.escapeHtml(node.id)}" style="${node.fontSize ? `--owb-font-size:${node.fontSize}px;` : ""}${node.zoom && node.zoom !== 1 ? `--owb-zoom:${node.zoom};` : ""}${node.textColor ? `--owb-text-color:${node.textColor};` : ""}"><div class="owb-node-inner"><header class="owb-node-head"><strong>${api.escapeHtml(node.title)}</strong><span class="owb-node-kind">${api.escapeHtml(nodeLabel(node))}</span></header><div class="owb-node-body">${body}</div></div><span class="owb-resize"></span></section>`;
      }
      ...
      function nodeDescription(node) { const rect = nodeRect(node); const position = `${node.floating ? "浮动" : "停靠"} · X${formatNum(rect.x + 1)} Y${formatNum(rect.y + 1)} · ${formatNum(rect.w)}×${formatNum(rect.h)}${node.collapsed ? " · 已折叠" : ""}`; return node.kind === "container" ? position : `${contentKinds[node.content]?.label || node.content}${node.content === "task" ? ` · ${viewLabels[node.view]}${node.query ? ` · ${node.query}` : ""}` : ""} · ${position}`; }
