/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"4.1.9","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-layout-body{position:relative;display:grid;place-items:center;height:100%;padding:4px;text-align:center}.owb-layout-open{width:100%;height:100%;padding:6px 28px 6px 8px;color:#fff;background:var(--accent);border:0;border-radius:8px;font-size:9px;font-weight:850;white-space:nowrap;touch-action:manipulation}.owb-layout-drag{position:absolute;z-index:7;right:7px;top:50%;display:grid;width:20px;height:30px;place-items:center;transform:translateY(-50%);color:rgba(255,255,255,.8);background:rgba(255,255,255,.15);border-radius:6px;cursor:grab;touch-action:none;user-select:none}.owb-layout-drag:active{cursor:grabbing}
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "4.1.9", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "布局按钮点击区与拖动手柄彻底分离，避免点击被拖动逻辑拦截" },
    activate(api) {
      ...
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "4.1.9", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function bindNode(element, node) {
        ...
        layoutButton?.addEventListener("pointerdown", (event) => event.stopPropagation());
        layoutButton?.addEventListener("click", (event) => { event.stopPropagation(); const wasLayout = config.mode === "layout"; config.mode = wasLayout ? "use" : "layout"; if (wasLayout) pushHistory(); editingId = null; drawer.classList.toggle("show", config.mode === "layout"); save(); renderWorkspace(); renderDrawer(); });
