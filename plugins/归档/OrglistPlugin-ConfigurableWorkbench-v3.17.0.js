/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"3.17.0","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const viewLabels = {
    ...
    completed: "已完成", "no-date": "无日期", habits: "习惯", quadrant: "四象限",
  ...
  function normalizeNode(value, index, ids) {
    ...
    const node = { id, kind, content, title: String(value?.title || (kind === "container" ? "容器" : contentKinds[content].label)).slice(0, 32), parent: String(value?.parent || "root"), x: clampFloat(value?.x, 0, 12 - w, 0), y: clampFloat(value?.y, 0, 240, 0), w, h: clampFloat(value?.h, 0.5, 24, kind === "container" ? 8 : 4), floating: !!value?.floating, locked: !!value?.locked, collapsible: !!value?.floating && !!value?.collapsible, autoCollapse: !!value?.floating && !!value?.collapsible && !!value?.autoCollapse, collapsed: !!value?.floating && !!value?.collapsible && !!value?.collapsed, cw: clampFloat(value?.cw, 0.1, 12, 2), ch: clampFloat(value?.ch, 0.1, 8, 1), cx: clampFloat(value?.cx, 0, 11.5, clampFloat(value?.x, 0, 11.5, 0)), cy: clampFloat(value?.cy, 0, 240, clampFloat(value?.y, 0, 240, 0)), showFrame: !!value?.showFrame, showLabel: value && Object.prototype.hasOwnProperty.call(value, "showLabel") ? !!value.showLabel : content === "task" };
    ...
    if (content === "layout") { node.floating = true; node.collapsible = false; node.autoCollapse = false; node.collapsed = false; }
    ...
    if (content === "task" || content === "mainList") { node.view = Object.prototype.hasOwnProperty.call(viewLabels, value?.view) ? value.view : "all"; node.query = String(value?.query || "").slice(0, 180); node.limit = clamp(value?.limit, 1, 200, 20); node.calendarCursor = validDate(value?.calendarCursor) ? value.calendarCursor : ""; node.filters = value?.filters && typeof value.filters === "object" ? value.filters : {}; node.filterButton = !!value?.filterButton; node.quadrantRules = { important: ["A", "AB", "ANY", "TAG"].includes(value?.quadrantRules?.important) ? value.quadrantRules.important : "A", urgent: ["0", "3", "7", "14", "OVERDUE", "TAG"].includes(value?.quadrantRules?.urgent) ? value.quadrantRules.urgent : "3" }; }
    return node;
  }
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "3.17.0", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "布局按钮加大易点；点击条目自动展开折叠的详情窗口" },
    activate(api) {
      ...
      function taskQueryOptions(node, view) {
        const calendar = isCalendar(view); const quadrant = view === "quadrant"; const wide = calendar || quadrant;
        const options = { view: wide ? "all" : view, query: node.query || "", limit: wide ? 200 : node.limit };
        const filters = node.filters || {};
        if (filters.status && filters.status !== "ALL") options.status = [String(filters.status)];
      ...
      function taskMarkup(node) {
        const view = node.view === "all" && node.filters?.view && viewLabels[node.filters.view] ? node.filters.view : node.view;
        const tasks = api.queryTasks(taskQueryOptions(node, view));
        const bar = node.content === "mainList" || node.filterButton ? filterBarMarkup(node) : "";
        if (isCalendar(view)) return bar + calendarMarkup(node, tasks);
        if (view === "quadrant") return bar + quadrantMarkup(node, tasks);
      ...
      function pointerInteraction(event, node, element, mode) {
        ...
        const move = (next) => { if (next.pointerId !== event.pointerId) return; const dx = Math.round((next.clientX - start.px) / columnStep / step) * step; const dy = Math.round((next.clientY - start.py) / rowStep / step) * step; const candidate = { x: start.x, y: start.y, w: start.w, h: start.h };
          ...
          else { const minStep = node.floating && node.collapsible && node.collapsed ? 0.1 : 0.5; candidate.w = Math.max(minStep, Math.min(12 - candidate.x, start.w + dx)); candidate.h = Math.max(minStep, Math.min(24, start.h + dy)); }
      ...
      function renderWorkspace() {
        ...
        canvas.querySelectorAll("[data-apply-filters]").forEach((button) => button.onclick = () => {
          ...
          const appView = document.querySelector("#nav [data-view].active")?.dataset?.view || lastSummary.view || "";
          const view = node.view === "all" ? appView : node.view;
      ...
      api.onUpdate((summary) => { lastSummary = summary; const detailNode = config.nodes.find((node) => node.content === "detail"); if (summary.selected && summary.detailOpen && detailNode && detailNode.collapsed) { detailNode.collapsed = false; save(); } renderWorkspace(); });
      renderWorkspace(); renderDrawer();
