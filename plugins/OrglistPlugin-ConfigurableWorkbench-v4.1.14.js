/* @orglist-plugin {"id":"orglist.configurable-workbench","name":"空白画布工作台","version":"4.1.14","type":"layout","hostApi":2} */
(function () {
  "use strict";
  ...
  const css = `
    ...
    .owb-layout-body{position:relative;display:grid;place-items:center;height:100%;padding:4px;text-align:center}.owb-layout-open{position:relative;z-index:6;width:100%;height:100%;padding:6px 28px 6px 8px;color:#fff;background:var(--accent);border:0;border-radius:8px;font-size:9px;font-weight:850;white-space:nowrap;pointer-events:auto}.owb-layout-drag{position:absolute;z-index:7;right:7px;top:50%;display:grid;width:20px;height:30px;place-items:center;transform:translateY(-50%);color:rgba(255,255,255,.8);background:rgba(255,255,255,.15);border-radius:6px;cursor:grab;touch-action:none;user-select:none}.owb-layout-drag:active{cursor:grabbing}
    ...
    .owb-mode-use{grid-template-columns:repeat(3,minmax(70px,90px)) repeat(9,minmax(0,1fr))}.owb-mode-use .owb-container>.owb-node-inner>.owb-node-head,.owb-mode-use .owb-core:not(.owb-core-task)>.owb-node-inner>.owb-node-head,.owb-mode-use .owb-core-layout>.owb-node-inner>.owb-node-head{display:none}.owb-mode-use .owb-node:not(.floating){overflow:visible;background:transparent;border:0;border-radius:0;box-shadow:none}.owb-mode-use .owb-container-body{padding:0;overflow:auto;background:transparent}.owb-mode-use .owb-container>.owb-node-inner>.owb-node-body{overflow:visible}.owb-mode-use .owb-core-task>.owb-node-inner>.owb-node-head{min-height:30px;padding:5px 8px;cursor:default}.owb-mode-use .owb-node.collapsed>.owb-node-inner>.owb-node-head{display:flex;height:100%;padding:7px 10px;border:0;border-radius:10px;cursor:pointer}.owb-mode-use .owb-resize,.owb-mode-use .owb-layout-drag{display:none}.owb-mode-use .owb-core-button .owb-node-body{padding:0}.owb-mode-use .owb-core-button .owb-node-body>button{width:100%;height:100%}.owb-mode-use .owb-core-layout{z-index:55}.owb-mode-use .owb-core-layout>.owb-node-inner>.owb-node-body{display:block!important}
  ...
  host.register({
    manifest: { id: pluginId, name: "空白画布工作台", version: "4.1.14", type: "layout", hostApi: 2, platforms: ["windows", "android"], description: "使用模式的布局入口改为普通点击按钮，仅布局模式显示拖动手柄" },
    activate(api) {
      ...
      const switchMode = (mode) => {
        const wasLayout = config.mode === "layout";
        config.mode = mode === "layout" ? "layout" : "use";
        if (wasLayout && config.mode === "use") pushHistory();
        editingId = null; drawer.classList.toggle("show", config.mode === "layout");
        save(); renderWorkspace(); renderDrawer();
      };
      const toggleMode = () => switchMode(config.mode === "layout" ? "use" : "layout");
      const openLayout = () => switchMode("layout");
      const exportHistory = () => {
        const payload = { app: "orglist.configurable-workbench", version: "4.1.14", exportedAt: new Date().toISOString(), defaultLayout: api.getSetting("defaultLayout", null), layoutHistory: api.getSetting("layoutHistory", []), config: { schema: 4, mode: config.mode, nodes: config.nodes } };
      ...
      function pointerInteraction(event, node, element, mode) {
        const allowDrag = config.mode === "layout";
      ...
      function bindNode(element, node) {
        ...
        if (layoutButton) layoutButton.onclick = (event) => { event.stopPropagation(); toggleMode(); };
      ...
      return { openLayout, deactivate() { clearTimeout(locateTimer); clearInterval(countdownTimer); document.removeEventListener("pointerdown", outsideCollapse); restoreRegions(); document.body.classList.remove("owb-blank"); } };
    },
  });
})();
